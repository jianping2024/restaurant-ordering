-- Soft-compat confirm_bill_split_payment: omit/empty payment_method stores NULL.
-- New Web HTTP still requires tender; fiscal/cash-drawer remain additive (CASH only).
-- Do not default empty to CASH (would falsify ledger and may kick drawer).
...
begin
  ...
  -- Empty / omitted → NULL (legacy clients & pre-tender Web). Non-empty must be enum.
  v_payment_method := upper(trim(coalesce(p_payment_method, '')));
  if v_payment_method = '' then
    v_payment_method := null;
  elsif v_payment_method not in ('CASH', 'CARD', 'MBWAY', 'MULTIBANCO', 'MIXED', 'OTHER') then
