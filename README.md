...
|------|------|
| `/` | 产品落地页 |
| `/auth/login` | **统一登录**（店主邮箱或员工登录名；一页 + `POST /api/auth/login`） |
...
| `/[slug]/staff/login` | 店内扫码别名（同款统一登录 UI，带店名上下文） |
| `/auth/staff/login` | **已合并** — 重定向至 `/auth/login` |
...
- **统一登录 `/auth/login`**（或 **店主**从后台侧边栏打开看板，**同一浏览器会话**无需再录员工密码）后，页面通过 **Supabase Realtime** 订阅 `orders` 与 `table_sessions`，并在进入时 **立即拉取** 最新数据（未满足则跳转到登录页）。
...
5. 在 `/dashboard/tables` 下载或打印 **桌位二维码**（店内登录二维码可指向 `/[slug]/staff/login`，与统一登录同款）
6. 员工打开 **`/auth/login`**（或店内扫码别名），用登录名 + 密码登录；首次或重置后须完成 **`/auth/staff/change-password`**
7. 再打开 **`/[slug]/kitchen`** 或 **`/dashboard/waiter`** 进入对应看板（角色不匹配会提示错误）
