...
| 顶栏 | Dashboard / Waiter 职员顶栏：内容行 `h-14` + `safe-area-inset-top/left/right`（`staffTopBarChrome`）；**FARVOO + 餐厅名**：`brand` 为唯一 `flex-1 min-w-0` 槽，店名截断（`title` 全名）；☰ 与 trailing（授权 + 角色）`shrink-0`；header `overflow-x-clip`；整行不得撑出文档横滑；`<lg` 仅 ☰ + 角色菜单，`≥lg` 品牌后 `mesa-chip-scroll` 横滑 nav；看板 lane / 桌台详情吸顶用 `waiterStaffStickyChrome` |
