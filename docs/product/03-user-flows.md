...
3. `decrementOrderItemWithAudit` 持久化（减至 0 写 `void_reason`，不进异常队列；楼面减菜写 `ITEM_QTY_DECREMENTED` 操作记录）
...
| `operation_logs` | 后厨 void：写入；楼面减菜：写 `ITEM_QTY_DECREMENTED` |
...
4. 成功后将来源桌 `remove` 出组；待结账等未参与桌保留；不因合并本身 dissolve 整组（移出后若已无成员则自动解散）

### 异常流程

| 情况 | 行为 |
|------|------|
| 来源桌在任一同行组 | 详情禁用转台/并台（不在 action API 层拒绝） |
| 并台目标在同行组 | 目标列表排除组内桌（看板本地 + `action-targets`） |
...
| 空会话结账 | `empty_session` |
| 自助餐人数未确认（成人+儿童合计为 0） | 400 `guest_count_required`；账单页底栏「呼叫结账」上方一条琥珀警示常驻；「呼叫结账」禁用；若仍触发客户端校验则 toast 同文案且不发请求 |
| 同行组内成员数 > 1 | 400 `party_merge_required`；账单页琥珀警示「请呼叫服务员并桌」；「呼叫结账」禁用；组内仅剩 1 桌（或不在组）才可成功 |
...
- 人数未确认时：底栏结账按钮上方琥珀警示一直显示；「呼叫结账」禁用，且不进入 `billing`
- 同行组内多桌时：同上位置显示并桌提示，「呼叫结账」禁用；并桌/移出后只剩一桌（或不在组）才可呼叫成功
- 服务员改人数后（成人+儿童 > 0）：提醒消失，按钮可点，可正常呼叫结账
- 呼叫结账后菜单不可加菜
- 结账台「待收」= 折后应收 − 已收合计
- 部分收款后恢复点餐：已收款项不丢失
- 全员付清后会话关闭、桌位可接新客
