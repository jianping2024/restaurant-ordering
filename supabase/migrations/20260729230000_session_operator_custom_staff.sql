-- App transfer/merge auth is capability-based (tables.transfer / tables.merge).
-- Operator RPC must accept active custom staff as well as preset floor roles.
...
COMMENT ON FUNCTION public.assert_restaurant_session_operator(uuid, uuid) IS
  'Transfer/merge operator gate: restaurant owner_id or active staff role in waiter|frontdesk|cashier|owner|custom. Fine-grained tables.transfer/merge is enforced in the app.';
