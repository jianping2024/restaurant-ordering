...
export const customerMenuBottomBarDockClass =
  `fixed bottom-0 left-1/2 z-30 ${CUSTOMER_MENU_SHELL_WIDTH_CLASS} -translate-x-1/2 border-t border-brand-border/80 bg-brand-card/95 backdrop-blur-md ${MESA_SURFACE.dock} pb-[max(0px,env(safe-area-inset-bottom))]`;
...
export const customerMenuBottomBarPrimaryActionClass =
  `${customerMenuBottomBarPrimaryActionBaseClass} transition-[filter] hover:brightness-110 active:scale-[0.98] ${MESA_SURFACE.selected}`;
