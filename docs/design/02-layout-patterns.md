...
| 分类 | 唯一 `CustomerMenuCategoryStrip`：全部一级横滑 + 右侧钉死「更多」（文本控件）；分类名完整显示（横滑消化长度，不单颗 truncate）；选中金底字用 `text-brand-on-gold`；展开时同一份 `topCategories` 两列 overlay 从条顶盖住横滑和「更多」（不留空行）；格子内 pill 唯一约束 `overflow-hidden` + `[overflow-wrap:anywhere]`，长名在圆角内换行、不画出框；点分类后回到横滑并把选中 chip `scrollIntoView` |
