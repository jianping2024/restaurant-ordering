...
- **Recommended dishes:** sole list `menu_recommended_items` + catalog `recommendedItemIds`; customer merchandising is one `CustomerRecommendedRail` poster card (4:3 photo + `formatMenuCatalogItemLabel` + `formatCustomerMenuItemPrice`; tap opens detail). Do not overlay `MenuItemAddButton`, reuse `MenuItemCard`, add `is_recommended` on `menu_items`, or a real/virtual `recommended` category. Dashboard add is one `POST` `{ menu_item_ids }` (array even for one dish); do not keep a parallel `menu_item_id` add body.
