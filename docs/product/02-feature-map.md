...
- 推荐不是真实分类、也不是 `menu_items` 布尔；顾客目录只带有序 `recommendedItemIds`；有上架推荐时分类条**下面**一条横滑海报轨（`CustomerRecommendedRail`：4:3 图 + 目录名 + 价，点卡进详情，无角上 `+`、不吸顶、无「换一批」），分类条不再插入虚拟「推荐」；下架推荐菜不出现在该轨
