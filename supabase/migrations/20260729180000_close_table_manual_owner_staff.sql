-- Allow store-owner staff (role = 'owner') on unpaid/force close, matching tables.force_close.
-- Operator set: restaurants.owner_id OR active staff role in frontdesk|owner (cashier stays on settled 关台结账).
...
BEGIN
  ...
  OR EXISTS (
    ...
    WHERE a.restaurant_id = p_restaurant_id
      ...
      AND a.role = ANY (ARRAY['frontdesk', 'owner']::text[])
...
COMMENT ON FUNCTION public.close_table_session_manual(
  uuid, uuid, uuid, text, boolean, text, text
) IS
  'Manual unpaid/force close. Operator: restaurant owner_id or active staff role frontdesk|owner. App enforces tables.force_close.';
