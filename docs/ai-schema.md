...
restaurants (id: uuid PK, name: text, slug: text unique, owner_id: uuid FK -> auth.users.id nullable [required when deployment_mode=cloud], owner_email: text nullable, deployment_mode: text [cloud|on_prem] not null default cloud, license_valid_until: timestamptz nullable, license_checked_at: timestamptz nullable, license_lease_until: timestamptz nullable, license_lease_token: text nullable, logo_url: text nullable, address: text nullable, phone: text nullable, plan: text [free|pro], kitchen_password: text, waiter_password: text, geo_latitude: double precision nullable, geo_longitude: double precision nullable, print_locale: text [zh|en|pt], country_code: char(2) not null default PT, print_agent_config: jsonb, feature_flags: jsonb default {}, kitchen_password_version: integer, waiter_password_version: integer, order_radius_meters: integer range 10..10000, buffet_friday_weekend_from: time nullable, buffet_service_mode: text [classic|sushi] not null default classic, guest_ordering_notice: jsonb default {enabled,title{pt,en,zh},body{pt,en,zh},updated_at}, permissions_version: integer not null default 0, suspended_at: timestamptz nullable, suspension_reason: text nullable, daily_business_report_enabled: boolean not null default false [Ops on_prem 经营日报上报], created_at: timestamptz)
...
| `close_table_session_settled(restaurant_id, table_id, closed_reason, closed_by_user_id?)` | authenticated, service_role | Settled cash/frontdesk close: open/billing; cancels unpaid splits; preserves orders (no invented paid split/ledger); closes session; not anon |
...
| `close_table_session_manual(restaurant_id, table_id, operator_user_id, closed_reason, confirm_close, unpaid_reason?, unpaid_reason_detail?)` | authenticated, service_role | Manual unpaid/force close. Operator: `restaurants.owner_id` or active staff `role` in `frontdesk\|owner`. App enforces `tables.force_close`; validates unpaid reason; returns audit snapshot (audit rows via `recordAudit`) |
...
- Settled close: `close_table_session_settled(...)` — advisory lock; open/billing; cancels unpaid splits; preserves orders; does not invent paid split/ledger; closes session.
