...
- 桌位分组：命名分组、组成员、备注；分组列表与组内桌位均拖拽手柄（@hello-pangea/dnd）调整顺序，松手一次提交 `ordered_ids` / `reorder_members`
- 桌位看板：空闲 / 用餐 / 待结账等状态展示
- **同行组（看板标记）**：看板用横滑楼面 lane tab，同行组入口紧跟最后一桌组（同一横滑条，选中 brand-gold；菜单顶区金底「创建」主按钮，下列已有同行组；点组项进入该组面板）；新建同行组 `sort_order` 追加（ASC，菜单顺序）；创建后进该同行组并焦点落在组名（默认名 `{login_name}-together-{N}`，店主无登录名用 `owner`；失焦提交；同店组名唯一、大小写不敏感）；仅开台用餐中桌可加入（空闲/待结账过滤；无可选桌时提示先开台）；进组后只在该组显示；**组内仅剩最后一桌时可呼叫结账**（成员数 > 1 时账单页提示先并桌，服务端 `party_merge_required`）；结账后仍留在组内、不进待结账置顶；关台/移出后自动退出同行组；若组内已无成员则自动解散该组（新建空组仍允许，不定时扫空组）；**组内桌详情禁用转台/并台**（仅 UI）；**组外并台目标列表排除组内桌**；**组头「一键合并」**：仅合并组内用餐中桌到展示序第一张用餐中桌，确认弹窗放大目标桌号；成功后来源桌移出组，待结账等未参与桌保留；用餐中不足两桌时提示不需要合并
...
- 预定 / 候位排队
- 按区域计费
- 同行组与打印业务联动（结账仍按会话独立；**组内多桌须先并桌/只剩一桌才可呼叫结账**；转台/并台：组内详情 UI 禁转/并、并台目标排除组内桌；组头一键合并走既有 `tables/action` merge，不在 action API 拒组内来源）
...
| 同行组 UI | `apps/web/src/components/waiter/WaiterBoardPartySections.tsx`、`WaiterBoardPartyLaneMenu.tsx` |
...
- 推荐不是真实分类、也不是 `menu_items` 布尔；顾客目录只带有序 `recommendedItemIds`；有上架推荐时分类条**下面**一条横滑海报轨（`CustomerRecommendedRail`：锁死 4:3 图槽 + 两行目录名槽 + 价齐底，点卡进详情，无角上 `+`、不吸顶、无「换一批」；金底上下同一 `py-*`，卡止于价格），分类条不再插入虚拟「推荐」；下架推荐菜不出现在该轨
...
- 首页概览：今日营业额、今日桌数（同卡含今日用餐人数大人/小孩）、当前用餐桌数（同卡含当前用餐人数大人/小孩）
...
- 呼叫结账前置：自助餐人数须已确认（成人+儿童 > 0）；账单页底栏「呼叫结账」上方一条琥珀警示（不透明底），人数改对且账单同步后消失；按钮禁用，点击与 API 均拦截；账单页靠进页/回前台/可见轮询拉权威单（顾客端无 orders Realtime）
...
- **屏上订单行菜名**（已点抽屉 / 账单 / 楼面已点 / 厨房看板 / 结账明细）跟界面语言，从该行快照三语字段选取；唯一函数 `resolveMenuItemLocalizedName`。纸面仍走 `menuLocalizedName` + `print_locale`。
...
- 查询：日期窗（默认近 7 天，可配置 7–90 天）+ `action_type` 筛选；侧栏「操作记录」；Cloud 与 on-prem 均可用
- 保留：`restaurants.operation_log_retention_days`（默认 7）；`resolveOperationLogRetentionDays` 统一驱动查询窗、日期选择与夜间 purge；现有 Vercel cron `/api/cron/nightly-close-sessions` 在 auto-close 门控之前执行 purge

### 业务边界

- 权限：`dashboard.operation_logs.view`（默认前台 + 店主，角色可配置）
- 配置：`settings.features.manage` → 功能管理「日志保留天数」（7–90，默认 7）
- 顾客发起的呼叫结账不记（仅员工）
...
| Lib | `apps/web/src/lib/operation-logs/*`（含 `retention-days.ts`）、`lib/audit/schedule-record-audit.ts`、`lib/audit/builders/staff-operations.ts` |
