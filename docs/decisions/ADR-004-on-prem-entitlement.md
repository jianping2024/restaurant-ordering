...
9. **Cross-DB claim (store install bridge)** — end-state (must ship; see handoff §1.4):
   ...
   - On-prem web **`/setup`**: sole claim UI (install code + owner password). One local apply-claim path: insert local `restaurants` with **the same UUID** as platform `restaurantId` (first claim), or **rebind** lease + platform license config and reset owner password when the local restaurant already has `owner_id` (recovery after lost license file). Persist check-in trio to host **`deploy/on-prem/license-state/platform.json`** (compose volume; survives web rebuild); env trio is bootstrap/fallback only. Missing config + unverifiable lease → fail-closed suspend. Then materialize.
