...
**服务员看板**角色类在 `waiterBoardType` / `WAITER_BOARD_LANE_CHROME`（`waiter-board-card-theme.ts`）。楼面 lane 与同行组下拉触发器 **同一 chrome**；选中为实心 `bg-brand-ink` + `text-brand-on-ink`（`WAITER_BOARD_SELECTED_EMPHASIS`，无 ring 双线）。桌卡壳为 `mesa-scroll-frame` + `is-dining` / `is-pending` / `is-free`（双线裱边）。桌卡展示唯一形状为 `WaiterBoardCardViewModel`（`metaChips` + 竖排 `statusLabel` + 唯一金边 `titleBadge`：`{ relation, tokens }` 单 pill 内竖排大人→小孩 token，勿再拼 `A3-C2` 横串）；KPI 为 `mesa-stat` + **唯一底色修饰** `WAITER_BOARD_KPI_SURFACE_CLASS`（`is-dining` / `is-pending` / `is-free` 与桌卡共用 CSS 实心底；`all` = `is-all` 品牌中性；选中只加粗金线，不用 ink 实心）。新建同行组 `sort_order` **append**（菜单列表按 ASC；条上不再为每组占一个 tab）。

| 角色 | 用途 |
|------|------|
| `pageTitle` | 看板页标题（`text-brand-ink`） |
| `kpiCount` / `kpiLabel` / `kpiHeadcountWing` / `kpiCountRowWithWings` + `WAITER_BOARD_KPI_SURFACE_CLASS` | 顶部 KPI（标签上、**居中大号**桌数下、色线；格底与桌卡同态；`kpiCount` = `text-4xl font-bold` + `text-center`；**仅「用餐中」** KPI 用 `kpiCountRowWithWings`：内容宽 A\|桌数\|C + **同一** `gap-x-1.5`（空挡相对桌数左右相等；禁止等宽翼槽——短 `C4` 会显得比 `A999` 离桌数更远）；翼面 = `text-[15px] font-medium text-brand-text`；其它 KPI 无翼） |
...
| `cardTitle` / `cardOpener` / `cardOpenerSlot` / `cardBadge` / `cardMeta` / `cardAmount` / `cardAmountSlot` / `cardCta` / `cardStatusSlot` / `cardStatus` / `cardRailBelow*` | 桌卡唯一排版面：桌号（display，body 面 `text-4xl`，≥2× 旧 `text-lg`）、金边角标（status rail `statusBadge`：`{ relation, tokens }`）、元信息行（`cardMeta` 固定一行 `h-4` + `flex-nowrap`，座位±用时）、**开桌人仅在横线下方**（`cardOpenerSlot` 固定 `h-5` 空/有名同高 + `cardOpener` 面）、金额（body 面 `text-[22px]` + `font-semibold` + `tabular-nums` + `brand-ink`；**不**用 `.mesa-money` / `font-heading` / 淡金/按态 `mesa-text-*`；状态只走壳/竖排/CTA）、金额槽 `cardAmountSlot`（空/有价同 `h-7`）、CTA、竖排状态（`cardStatusSlot` 固定 `h-[4.5rem]` + `.mesa-status-vertical`）、右侧 `cardRailBelow` 始终占位（无 badge 时空槽，与 opener+amount 镜像）；禁止在卡片组件另写平行 badge/meta/cta/status/opener class；`WAITER_BOARD_CARD_THEME` 无 `amount` 字段 |
...
- 待结账 `checkout`：warning（琥珀）→ `is-pending`
- 用餐中 `dining`：danger（红）→ `is-dining`
