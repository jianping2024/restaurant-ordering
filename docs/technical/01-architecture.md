...
| `bill-split-*` | 分单算法与校验 |
| `buffet-*` | 自助餐开台与计价 |
| `order-item-void/` | 退菜持久化、审计 |
| `order-item-decrement/` | 楼面减菜权限（`decrement-policy.ts`） |
| `table-session/` | 关台等服务 |
...
| 服务员/厨房/结账实时 | `useRestaurantStaffEntryReconcile`（进页 + 回前台拉权威读模型）+ `useRestaurantRealtimeRefresh`（表面 active 时 `postgres_changes` 门铃 → 权威 GET；waiter board 仅列表可见时 active） |
...
```text
Browser /[slug]/menu
  → RSC loadCustomerSessionContext（首屏 session + recent_orders）
  → client useCustomerSessionContext reconcile GET .../customer/session
