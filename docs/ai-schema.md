...
| `close_table_session_settled(restaurant_id, table_id, closed_reason, closed_by_user_id?)` | authenticated, service_role | Settled cash/frontdesk close: open/billing; cancels unpaid splits; writes paid split + collected payment when payable > 0; preserves orders (no void); closes session; not anon |
...
- Settled close: `close_table_session_settled(...)` — advisory lock; open/billing; cancels unpaid splits; writes paid split + ledger when payable > 0; preserves orders; closes session.
