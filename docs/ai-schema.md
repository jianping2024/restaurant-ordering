...
| `close_table_session_manual(restaurant_id, table_id, operator_user_id, closed_reason, confirm_close, unpaid_reason?, unpaid_reason_detail?)` | authenticated, service_role | Owner/frontdesk manual force-close; validates unpaid reason; returns audit snapshot (audit rows written in app via `recordAudit`) |
