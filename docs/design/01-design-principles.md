...
| `kpiCount` / `kpiLabel` / `kpiHeadcountWing` / `kpiCountRowWithWings` + `WAITER_BOARD_KPI_SURFACE_CLASS` | 顶部 KPI（标签上、**居中大号**桌数下、色线；格底与桌卡同态；`kpiCount` = `text-4xl font-bold` + `text-center`；**仅「用餐中」** KPI 用三栏 `kpiCountRowWithWings` 在桌数左右挂 `buffetHeadcountTokens` 的 A/C，空侧占位不挤中轴；其它 KPI 无翼） |
