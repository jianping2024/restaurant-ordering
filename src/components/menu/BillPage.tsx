...
import type { Order, SplitMode, SplitResult } from '@/types';
...
export function BillPage({ restaurant, tableNumber, orders }: Props) {
  ...
  // 仅统计“已完成”的菜品（支持菜品级出餐；兼容旧数据：订单 done 视为菜品已完成）
  const allItems = orders.flatMap(o => o.items
    .filter(item => (item.item_status || (o.status === 'done' ? 'done' : 'pending')) === 'done')
    .map((item, idx) => ({
      ...item,
      order_id: o.id,
      key: `${o.id}-${idx}`,
