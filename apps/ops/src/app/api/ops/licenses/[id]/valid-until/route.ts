...
import { setRestaurantLicenseValidUntilDate } from '@/lib/license-control';
...
export async function POST(req: Request, context: RouteContext) {
  ...
  const result = await setRestaurantLicenseValidUntilDate(admin, id, body.date);
  if (!result.ok) {
  ...
  await writePlatformAudit(admin, {
    ...
    action: 'license.set_valid_until',
    targetType: 'restaurant',
    targetId: id,
    restaurantId: id,
    metadata: { date: body.date, licenseValidUntil: result.licenseValidUntil },
  });
