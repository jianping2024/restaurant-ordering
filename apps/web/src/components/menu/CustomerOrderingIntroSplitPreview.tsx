...
type Props = {
  lang: UILanguage;
  intro: CustomerOrderingIntroCopy;
};

const PREVIEW_AMOUNTS = ['12,50', '11,50'] as const;

export function CustomerOrderingIntroSplitPreview({ lang, intro }: Props) {
    ...
    <div className="rounded-xl border border-brand-border bg-brand-bg/60 p-3">
      ...
      <div className="mb-2 border-t border-brand-border pt-2">
        ...
        <div className="space-y-1">
          <div className="flex items-center justify-between text-[11px] text-brand-text">
            <span>{intro.previewGuest1}</span>
            <span className="tabular-nums font-medium text-brand-gold">€{PREVIEW_AMOUNTS[0]}</span>
          </div>
          <div className="flex items-center justify-between text-[11px] text-brand-text">
            <span>{intro.previewGuest2}</span>
            <span className="tabular-nums font-medium text-brand-gold">€{PREVIEW_AMOUNTS[1]}</span>
          </div>
        </div>
      </div>
