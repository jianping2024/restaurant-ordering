...
| **出品屏 · 按档口（`print_stations`）下发（主按钮）** | **已确认**：**出品屏**（店内终端；**路由名不强制**「厨房」，示例 **`/[slug]/kitchen`**）**主操作**为 **单次点击「打印出品」**（或 **「出单」** 等统一文案）：**一次 API 请求** → 服务端对该 **订单行集** 按 **`effective_station_id`**（来自 **品/类目 → `print_stations`**）分组，**每个有行的 `print_station` 各 `insert` 一条** **`station_ticket`**（**多条 `print_jobs`，一次用户意图**）。**N 个 configured 档口** 即 **最多 N 条任务**。**口径**：**主路径 = 一次点击、多 `print_station` 各一条任务**；**不再**采用「**两个并列主按钮**（如按旧习惯拆后厨/酒水）与主路径二选一」的说法。**行集范围** 与 **加单批次** 见 **出品与加单批次**。**「仅重打某一 `print_station`」**（请求体带 **`print_station_id`**，只入队 **一条** `station_ticket`）：**首期不承诺 UI / 不与「厨房」场景绑定**，数据与 API 形态 **兼容**即可；若产品上需要，在 **出品屏「更多」** 或 **P2** 再开（见 **入队幂等**）。 |
| **出品与加单批次** | **受众**：**分批、按档口的 `station_ticket` 给店内出品人员看**，**不是**「顾客每批都要收一张」的对外小票；**顾客** 以 **`order_receipt`** 为 **最终纸本**（见 **票据受众**）。顾客 **多次下单 / 加单** 时，订单行按 **`batch_id`** 分批（见 `research.md`）。**已确认**：**顾客每新下一批**，**各相关 `print_station`** 都须能 **凭本批行打出品联**（**一条 `station_ticket` 只含一个 `print_station` + 一个 `batch_id` 内的行**）。**`station_ticket.payload` 须含 `batch_id`**（推荐；亦可与 **`line_item_ids`** 快照并存以便审计），**`lines` 仅该批、且归属本档口**；**主路径默认不**把 **已出过品联的历史批次** 再打一遍。**「当前待出」**（主按钮默认范围）：该 **`order_id`** 下 **尚未出品打印过的最早 `batch_id`**；若出品屏 UI **显式选中某批**，以 **选中批** 为准。**一批一动作**：一次主按钮请求 **只消费一个目标 `batch_id`**，在 **N 个有行档口** 上拆成 **N 条任务**；若 **多批积压**，需 **多次点击**（每批一次）或日后 **队列合并**（**P2**）。**「全单重打」**：次级入口，`payload` 须标 **`reprint_scope: full_order`**（或等价）并重冻行集。**（可选 P2）**：顾客提交后 **自动入队** `station_ticket`（须幂等、代理离线等策略）。**「本批是否已打」（首期已定，无另行产品偏好）**：**仅以 `print_jobs`（`type=station_ticket`）为事实来源**，**不**首期在订单行表加「已打出品联」列（行级回写列 **P2** 再说）。**单档口、单批「主路径已打出」**：存在 **`status='done'`** 且 **`payload.batch_id`、 `payload.print_station_id` 与当前批/档口一致**，且 **`payload` 未带 `reprint_scope: full_order`**（全单重打等 **带** `full_order` 的 **`done`** **不计入**「该批该档口主路径已齐」，避免与默认「待出」**误判**）。**「当前待出」默认**：同一 **`order_id`** 下按 **`batch_id` 升序** 取 **最早** 一批，使该批内 **凡有出品行的每个 `print_station_id`** 均 **尚不存在**上述「主路径已打出」的 **`done`** 行（即该批对各档口 **未齐套**）。**防重复入队**：出品屏 **debounce**；**可选** `insert` 前若已有 **同 `order_id`+`batch_id`+`print_station_id` 且 `status in ('pending','processing')`** 则 **拒绝或跳过**（实现二选一）。 |
...
| **入队幂等（`print_jobs`）** | **第一期**：**`order_receipt` / `pre_bill`**：**每次点击「打印」新建一条** `print_jobs`。**`station_ticket`**：**出品屏主按钮**（见 **出品屏 · 按档口（`print_stations`）下发**）**一次操作**允许 **同一请求内 `insert` 多条**（**每个有行的 `print_station` 一条**），仍视为 **单次用户意图**。**「仅重打单一 `print_station`」**（单请求 **一条** `station_ticket`，`payload` 带目标 **`print_station_id`**）：**首期不做专门入口**，列 **P2**（与 **出品屏 · 按档口下发** 一致）。**不**做同单短时间服务端合并 / **不**要求 `client_request_id`。**理由**：审计清晰；实现简单。**误触连点**：**前端 debounce**（如 300–500ms）或按钮 loading。**若日后**弱网重试导致重复入队、或产品硬性「N 秒内同单只打一张」，再引入 **`client_request_id`（或 `Idempotency-Key`）** + 短窗唯一约束（**P2** 或独立迭代）。 |
...
1. **`OrdersHistoryManager`、出品屏**（与 **`print_stations`** 绑定之终端；路由名不强制 `kitchen`）、**账单（预结）** 等  
   - **三类 `print_jobs` 同一套路径**（**有代理时**）：点击「打印」→ **已登录** `POST /api/print-jobs`（或 Server Action）→ **`insert print_jobs`**（`type` 为 `order_receipt` / `station_ticket` / `pre_bill`），**服务端**写 **`payload` 冻结快照**（见 **§5.4**）；**`locale`** = **`restaurants.print_locale`**。由 **自建代理** 消费队列、**RAW 热敏**出纸。**无代理时** 走 **`window.print()` 兜底**（见 **已确认 · 无代理时打印**）；**不**把「仅浏览器」当目标态主路径。  
   - Toast：「已发送到打印队列」。  
   - **档口出品联 / 预结**：入队 **`station_ticket`** 时 **`payload` 行集** 按 **`batch_id`（或请求指定批）** 与 **`effective_station_id`（品 `print_station_id` 优先于类目）** 分组，**每组一条任务**（见 **已确认 · 出品分流与 `print_stations`**、**出品与加单批次**）。**出品屏主路径**：**一次点击 → 同一目标批、多 `print_station` 各一条任务**（见 **已确认 · 出品屏 · 按档口（`print_stations`）下发**）；**服务端分组规则** 与表头决策一致。账单页入队 **`pre_bill`**；**`station_ticket`** 版式由 **`payload.ticket_layout`** 选模板，与 **`order_receipt` / `pre_bill`** 分离。

2. **幂等与重复打印**（见 **已确认产品与场景 · 入队幂等**）  
   - **第一期**：**`order_receipt` / `pre_bill`**：**每次点击** 新增 **一条** `print_jobs`；**`station_ticket`**：**主按钮「打印出品」** 一次点击可 **多条** `print_jobs`（每档口一条）。**前端**对打印按钮 **debounce** 减轻连点。  
...
| **2026-05-14** | **`print_jobs` 表**（`station_ticket` / 预留 `order_receipt`·`pre_bill`）、**`restaurants.print_locale`**（默认 `pt`）、**Realtime 发布**；**服务员桌台详情「打印出品」** → `POST /api/restaurants/[slug]/station-tickets`（**已登录员工会话 / 服务端鉴权**）按 **待出批次 + 档口** 写入队列；入队逻辑见 `src/lib/station-ticket-enqueue.ts`。 |
...
- [x] **`print_jobs`（首期：`station_ticket` 入队 + `print_locale`）** — `supabase/migrations/20260514120000_print_jobs_and_print_locale.sql`；**服务员 `/[slug]/waiter/[table]`「打印出品」** → `POST /api/restaurants/[slug]/station-tickets`（**配对 / claim / 代理消费仍待办**）  
...
- [ ] **订单小票** + **`station_ticket`（厨房出品屏等其余入口）** + **预结单** 入队入口与代理模板（**全票单一 `locale`**，见 **已确认产品与场景**）  
- [x] **入队策略（服务员出品）**：每次点击 **新建** `print_jobs`（按档口多条）；**约 450ms 防抖** + loading；**无** `client_request_id`（见 **入队幂等**）  
**安装包端**  
