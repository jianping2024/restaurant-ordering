...
- **Menu dish list row:** sole on-screen title `formatMenuCatalogItemLabel`; body column stacks identity → meta → trailing `€` + narrow availability toggle + `MenuItemListRowOverflowMenu` (edit/delete). Do not put price/toggle/⋯ on the same horizontal band as the dish name; do not pack PT·EN·ZH into one truncate line or inline ✎/× beside the toggle.
