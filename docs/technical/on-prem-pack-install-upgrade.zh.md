...
**内置超管（prem only，店主影子）：** 安装脚本会 `POST /api/setup/ensure-prem-builtin-admin`。登录名 `admin`，默认密码 `centos(123)`。不改 `owner_id`、不进员工列表；认领前不可登录，认领后与店主同一套身份（`loadOwnerRestaurantForUser`）。
