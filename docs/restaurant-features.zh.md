...
| `bill_sync_to_fiscal` | **关闭** | 勾选后结账页与桌台详情出现「同步关台」（先云端挂单，成功后再关台结账）；未勾选时入口隐藏且入队 API 拒绝。分单/打票在 Agent 本机。契约见 [`technical/farvoo-fiscal-bill-sync-api.zh.md`](./technical/farvoo-fiscal-bill-sync-api.zh.md) |
