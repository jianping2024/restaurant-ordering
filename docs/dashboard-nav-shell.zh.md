...
| 状态 | Grid 第一列（侧边栏） | 第二列（主内容） |
|------|----------------------|------------------|
| 展开（默认） | `16rem` | `minmax(0, 1fr)` |
| 收起 | `4.5rem` | `minmax(0, 1fr)` |

**桌面 `lg+`**：`DashboardShell` 使用 **CSS Grid 两列**，侧边栏 `lg:relative lg:sticky lg:w-full` 在文档流中占位；主内容在第二列，**不会被 fixed 层遮盖**。

**移动 `< lg`**：block 布局 + `fixed` drawer；主内容全宽 + `pt-20`。

- 收起状态写入 `localStorage` 键 `mesa:dashboard-nav-collapsed`（`1` = 收起）。
- SSR / hydration 默认展开；`grid-template-columns` 随 `collapsed` 过渡。

> **禁止**回退到 `fixed sidebar + main margin-left` 或 `margin-left: var(--…)`：sidebar 脱离文档流时，margin 一旦未生效就会遮盖内容（2026-06 线上 bug 根因）。
