...
| **账单编辑态** `BillPage` | 无常驻轮询。进入页 / 从后台回到前台时 `syncOrders` → `customer/bill?scope=live`（只刷新活跃会话、全量账单订单、同行组人数）；呼叫结账前 `resolveFreshBill` 做权威校验（15s 内刚同步过可去重）。同桌加菜等变化若未触发上述事件，提交前会发现并 toast，不静默按旧单结账。 |
| **账单成功页** `BillPage` | 无自动检测。停在「账单已提交、请到前台支付」时，顾客点 **「刷新页面」** 整页重载；服务端 `bill/page.tsx` 按最新 session 决策（关台 → 菜单空台；恢复点单 → 可编辑账单）。 |
