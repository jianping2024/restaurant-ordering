-- Multi-package buffet merge: consolidate per buffet_id, keep distinct packages separate.
...
begin
  ...
  for v_buffet_rec in
    select
      bl.buffet_id,
      coalesce(sum(bl.adults), 0)::int as adults,
      coalesce(sum(bl.children), 0)::int as children
    from (
      select
    ...
    group by bl.buffet_id
    having coalesce(sum(bl.adults), 0) + coalesce(sum(bl.children), 0) > 0
  loop
    v_buffet_id := v_buffet_rec.buffet_id;
    v_adults := v_buffet_rec.adults;
    v_children := v_buffet_rec.children;
    ...
    v_merged_lines := v_merged_lines || jsonb_build_array(v_merged_line);
  end loop;

  if jsonb_array_length(v_merged_lines) > 0 then
    for v_order_rec in
      select o.id, o.items
      from public.orders o
      where o.restaurant_id = p_restaurant_id
    ...
    if v_carrier_order_id is null then
      v_new_total := public.recalc_order_total_from_items(v_merged_lines);
      ...
      v_new_items := coalesce(v_carrier_items, '[]'::jsonb) || v_merged_lines;
      v_new_total := public.recalc_order_total_from_items(v_new_items);
      update public.orders
      set items = v_new_items,
          total_amount = v_new_total,
