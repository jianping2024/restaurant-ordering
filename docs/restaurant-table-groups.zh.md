...
| 下载 PNG | 仍为纯 QR 图（`qrcode` 库生成，**不含**文字）；分组名仅屏幕卡片与打印排版 |
...
由当前 **单一平铺网格** 改为：**待结账快捷区（§6.0.1）+ 按分区分组**：

```
[ 分组 A 名称 ]          ← section 标题（group.sort_order）
  ┌──┐ ┌──┐ ┌──┐
  │桌│ │桌│ │桌│        ← 组内桌位卡片，sort_order
  └──┘ └──┘ └──┘

[ 分组 B 名称 ]
  ...

[ 未分组 ]               ← 固定文案 i18n；仅当有未分组桌时显示
  ┌──┐ ┌──┐
  │桌│ │桌│
  └──┘ └──┘
```
