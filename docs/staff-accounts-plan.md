...
> **现行登录（已覆盖本节「双入口」描述）：** 店主与员工统一走 **`/auth/login`** + `POST /api/auth/login`。`/auth/staff/login` 已重定向到统一入口；`/{slug}/staff/login` 仅为同款店内扫码别名。本地/UAT 浏览器登录一律用 `/auth/login`。下文「双入口」保留为历史实施记录。
...
- 校验规则与店主设密一致：sole `accountPasswordPolicyError`（≥8、字母+数字、禁弱口令、不可等于登录名）。
