-- Atomic checkout payment confirmation: row-lock bill_splits, merge paid flags, optionally close session.
...
begin
  ...
  return jsonb_build_object(
    ...
    'should_print_split', v_split.session_id is not null and v_len > 1,
    'should_close_session', v_all_paid and v_split.session_id is not null
  );
exception
  when others then
    return jsonb_build_object(
...
revoke all on function public.confirm_bill_split_payment(uuid, uuid, integer, numeric) from anon;
grant execute on function public.confirm_bill_split_payment(uuid, uuid, integer, numeric) to authenticated, service_role;
