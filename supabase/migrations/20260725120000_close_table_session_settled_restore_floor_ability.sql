-- Restore floor 关台结账 ability on settled close:
-- same close eligibility as former operational checkout-close (cancel unpaid splits,
-- allow unfinished kitchen work), but preserve order amounts and write settlement
-- so revenue qualifies. Drop kitchen / unpaid-split / partial-ledger reject gates.
