...
| 空会话结账 | `empty_session` |
| 自助餐人数未确认（成人+儿童合计为 0） | 400 `guest_count_required`；账单页分单区上方 + 底栏「呼叫结账」上方琥珀警示常驻；「呼叫结账」禁用；若仍触发客户端校验则 toast 同文案且不发请求 |
...
- 人数未确认时：分单区上方 + 底栏结账按钮上方琥珀警示一直显示；「呼叫结账」禁用，且不进入 `billing`
