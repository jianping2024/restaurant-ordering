-- Snapshot opener label at open time so board/live read a column (no resolve-on-read).
alter table public.table_sessions
  add column if not exists opened_by_name text;

comment on column public.table_sessions.opened_by_name is
  'Opener label stamped at session open (same ladder as resolveStaffOperatorDisplayName / Auth username). Board and history read this; do not re-resolve from opened_by_user_id for display.';

-- Backfill from staff accounts (login preferred, else display — matches resolveStaffOperatorDisplayName).
update public.table_sessions ts
set opened_by_name = coalesce(
  nullif(btrim(rsa.login_name), ''),
  nullif(btrim(rsa.display_name), '')
)
from public.restaurant_staff_accounts rsa
where ts.opened_by_name is null
  and ts.opened_by_user_id is not null
  and rsa.restaurant_id = ts.restaurant_id
  and rsa.user_id = ts.opened_by_user_id
  and coalesce(
    nullif(btrim(rsa.login_name), ''),
    nullif(btrim(rsa.display_name), '')
  ) is not null;

-- History feed payload includes stamped opener name.
  ...
  WITH closed_rows AS (
    SELECT
      ...
      jsonb_build_object(
        ...
        'opened_by_name', ts.opened_by_name,
