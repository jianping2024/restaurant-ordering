-- Pick the time slot whose window is closest to "now" when the clock is outside all windows,
-- among slots that have an active price rule for this buffet + calendar + date.
-- Still prefers a slot that strictly contains the current time (distance 0).
