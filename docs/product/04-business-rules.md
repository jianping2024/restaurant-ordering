...
- **人数前置**：`checkout/request` 要求会话订单 active `buffet_base` 成人+儿童合计 > 0，否则 `guest_count_required`；账单页底栏结账按钮上方一条琥珀警示常驻，「呼叫结账」禁用，直至人数改对且客户端账单同步到最新单
...
- 来自会话订单中 active `buffet_base` 的成人/儿童数（`sessionGuestCounts` / `aggregateBuffetHeadcountForOrders`）
- 无 buffet 行 → 0
- 首页「今日用餐人数」仅统计与「今日桌数」同集合的会话；「当前用餐人数」仅统计 open|billing
