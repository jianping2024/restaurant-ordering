...
| 顶栏 | Dashboard / Waiter 职员顶栏：内容行 `h-14` + `safe-area-inset-top/left/right`（`staffTopBarChrome`）；**FARVOO + 餐厅名**：`brand` 为 `flex-1` + logo 地板 `STAFF_TOP_BAR_BRAND_MIN_CLASS`（`min-w-[7rem]`）+ `overflow-hidden`，店名唯一类 `restaurantNameClassName`（`text-base`/`sm:text-lg` + `text-brand-text`，`title` 全名截断）；右侧 cluster `max-w-[calc(100%-7rem)]`（与地板同一 `7rem`）；☰ 与 trailing（授权 + 角色）`shrink-0`；窄屏 trailing 文案宽度唯一 token：`STAFF_TOP_BAR_TRAILING_TEXT_MAX_CLASS`；**禁止**用 header `overflow-x-clip` 藏溢出；整行靠 flex 收缩，不得撑出文档横滑；故意横滑只走 `.mesa-chip-scroll`；`<lg` 仅 ☰ + 角色菜单，`≥lg` 品牌后 **bounded** chip 横滑 nav（`max-w-full` 于 cluster 内）；看板 lane / 桌台详情吸顶用 `waiterStaffStickyChrome` |
