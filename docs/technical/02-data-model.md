...
- 服务员看板「同行组」运行时标记（客人一起）；与楼面分组正交；组内桌不可自己发起转台/并台，组外并台目标排除组内桌；组内成员数 > 1 时禁止呼叫结账（须先并桌/只剩一桌）
- 每桌最多属于一个同行组；仅开台用餐中（看板 `dining`）可加入；展示归属优先于待结账置顶
- 同店组名唯一（`lower(btrim(name))`）；默认名 `{login_name}-together-{N}`（店主无登录名用 `owner`）；创建后可改名（失焦提交）
- 关台成功后清除该桌成员关系；移出后若组内已无成员则自动解散该组
