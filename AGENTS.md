...
- **Menu dish list row:** sole on-screen title `formatMenuCatalogItemLabel`; identity (name+meta) separate from actions. Mobile stacks name → meta → action row (`€` left, thin toggle + text edit/delete right). Desktop keeps identity left and actions right-aligned. Do not put price/toggle/actions on the same band as the dish name; do not keep a `⋯` overflow menu or PT·EN·ZH one-line truncate for the list title.
