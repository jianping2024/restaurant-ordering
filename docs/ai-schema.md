...
bill_sync_jobs (id: uuid PK, restaurant_id: uuid FK -> restaurants.id, request_id: uuid, source_system: text [farvoo], source_sale_id: uuid, table_display_name: text, scope_type: text [whole_table|split], payload: jsonb, status: text [pending|processing|succeeded|failed], error_code: text nullable, error_message: text nullable, document_id: text nullable [Agent invoices.id after auto_issue; reprint key], invoice_no: text nullable [Agent invoice_no copy], created_by: uuid nullable, created_at: timestamptz, updated_at: timestamptz; UNIQUE(restaurant_id, request_id); Realtime + print_agent SELECT RLS)
...
restaurants (id: uuid PK, name: text, slug: text unique, owner_id: uuid FK -> auth.users.id nullable [required when deployment_mode=cloud], owner_email: text nullable, deployment_mode: text [cloud|on_prem] not null default cloud, license_valid_until: timestamptz nullable, license_checked_at: timestamptz nullable, license_lease_until: timestamptz nullable, license_lease_token: text nullable, license_offline_grace_days: integer not null default 7 range 1..365, logo_url: text nullable, address: text nullable, phone: text nullable, plan: text [basic|pro], kitchen_password: text, waiter_password: text, geo_latitude: double precision nullable, geo_longitude: double precision nullable, print_locale: text [zh|en|pt], country_code: char(2) not null default PT, print_agent_config: jsonb, feature_flags: jsonb default {}, kitchen_password_version: integer, waiter_password_version: integer, order_radius_meters: integer range 10..10000, order_cooldown_seconds: integer not null default 5 range 5..60, operation_log_retention_days: integer not null default 7 range 7..90, buffet_friday_weekend_from: time nullable, buffet_service_mode: text [classic|sushi] not null default classic, guest_ordering_notice: jsonb default {enabled,title{pt,en,zh},body{pt,en,zh},updated_at}, permissions_version: integer not null default 0, menu_catalog_version: integer not null default 0, suspended_at: timestamptz nullable, suspension_reason: text nullable, created_at: timestamptz)
restaurant_installations (id: uuid PK, restaurant_id: uuid FK -> restaurants.id, install_code_hash: text unique, checkin_secret_hash: text nullable, status: text [pending|claimed|revoked], expires_at: timestamptz, consumed_at: timestamptz nullable, claimed_at: timestamptz nullable, revoked_at: timestamptz nullable, last_checkin_at: timestamptz nullable, created_by: uuid FK -> auth.users.id nullable, created_at: timestamptz; unique one pending and one claimed per restaurant; RLS on, no anon/auth policies — service_role/ops only)

table_sessions (id: uuid PK, restaurant_id: uuid FK -> restaurants.id, status: text [open|billing|closed], opened_at: timestamptz, closed_at: timestamptz nullable, merge_into_session_id: uuid FK -> table_sessions.id nullable, closed_reason: text nullable, closed_by_user_id: uuid FK -> auth.users.id nullable, opened_by_user_id: uuid FK -> auth.users.id nullable, opened_by_name: text nullable, table_id: uuid FK -> restaurant_tables.id, settled_payable_amount: numeric nullable)

table_order_rounds (id: uuid PK, restaurant_id: uuid FK -> restaurants.id, session_id: uuid FK -> table_sessions.id, table_id: uuid FK -> restaurant_tables.id, status: text [collecting|pending_confirm|cooldown|closed|finalize_failed], guest_count_snapshot: integer not null, per_person_cap: integer not null, submit_request_id: uuid nullable, submit_requested_at: timestamptz nullable, submit_deadline_at: timestamptz nullable, defer_used_at: timestamptz nullable, defer_cooldown_until: timestamptz nullable, cooldown_until: timestamptz nullable, append_client_request_id: uuid nullable, created_at: timestamptz, updated_at: timestamptz; partial unique active round per session — see migration; contract: docs/product/sushi-round-ordering.zh.md; trigger closes non-closed rounds when table_sessions.status → billing|closed; REPLICA IDENTITY FULL for filtered Realtime on session_id; anon/authenticated SELECT via table_session_is_open_or_billing(session_id) SECURITY DEFINER — do not EXISTS-read table_sessions under nested RLS)

table_order_round_lines (id: uuid PK, round_id: uuid FK -> table_order_rounds.id, menu_item_id: uuid FK -> menu_items.id, qty: integer not null, guest_client_id: uuid not null, note: text not null default '', added_at: timestamptz, UNIQUE(round_id, menu_item_id, guest_client_id))
...
| `confirm_bill_split_payment(restaurant_id, bill_split_id, person_index, collected_amount?, created_by_user_id?, payment_method?)` | authenticated, service_role | SECURITY DEFINER checkout; `payment_method` optional (omit/empty → ledger NULL for legacy clients); non-empty must be CASH\|CARD\|MBWAY\|MULTIBANCO\|MIXED\|OTHER else `invalid_payment_method`; appends `session_collected_payments`; rejects overpay; reconciles `result.paid`; closes session when settled; returns `collected_payment_id` + `payment_method`; advisory lock per session; not anon. New Web HTTP still requires tender; CASH drawer enqueue is app-side only when method is CASH. |
...
| `close_table_session_settled(restaurant_id, table_id, closed_reason, closed_by_user_id?, settled_payable_amount?)` | authenticated, service_role | Settled cash/frontdesk close: open/billing; cancels unpaid splits; preserves orders (no invented paid split/ledger); writes `settled_payable_amount`; closes session; not anon |
...
| `close_table_session_manual(restaurant_id, table_id, operator_user_id, closed_reason, confirm_close, unpaid_reason?, unpaid_reason_detail?)` | authenticated, service_role | Manual unpaid/force close. Operator: `restaurants.owner_id` or any active staff. App enforces `tables.force_close`; validates unpaid reason; returns audit snapshot (audit rows via `recordAudit`) |
| `transfer_table_session(restaurant_id, from_table_id, to_table_id, operator_user_id?)` | authenticated, service_role | Move open session between tables; operator via `assert_restaurant_session_operator` (owner_id or active staff waiter\|frontdesk\|cashier\|owner\|custom). App enforces `tables.transfer`. |
| `merge_table_sessions(restaurant_id, source_table_id, target_table_id, operator_user_id?)` | authenticated, service_role | Merge two table sessions; buffet lines merge per `buffet_id` (same package sums headcount, different packages stay separate); same operator gate as transfer. App enforces `tables.merge`. |
| `assert_restaurant_session_operator(restaurant_id, operator_user_id)` | authenticated, service_role | Transfer/merge operator gate: restaurant `owner_id` or active staff role in waiter\|frontdesk\|cashier\|owner\|custom |
...
- Retention: `restaurants.operation_log_retention_days` (7–90, default 7). `resolveOperationLogRetentionDays` drives list query window, date picker bounds, and nightly purge cutoff per restaurant.
- Purge: existing Vercel cron `/api/cron/nightly-close-sessions` runs `purgeExpiredOperationLogs` before the auto-close due gate (cloud + on-prem).
...
- restaurant_staff_accounts_login_name_key: unique btree(login_name)
...
- `restaurants.print_agent_config` JSON: `{ schedule?, poll?, credential_ttl_days?, default_receipt_station_id?, station_slip_show_category_group?, han_bitmap_font_px?, kitchen_ready_after_minutes? }` — writers merge owned slices onto the existing document (schedule/poll vs TTL+category vs receipt station vs kitchen ready-after). `credential_ttl_days` integer **1–365** (default **365**) sets JWT `exp` and `print_agent_devices.valid_until` on the next successful `claim` (功能管理). `kitchen_ready_after_minutes` integer **3–30** (default **10**) is edited only on 功能管理.
- Platform ops (`@mesa/ops`): `platform_admin_accounts` links Farvoo staff to `auth.users`; `platform_admin_audit_log` records cross-tenant actions. **No RLS policies** — accessed via service role only after session check in ops API. Restaurant suspend/resume sets `restaurants.suspended_at` + `suspension_reason` (null on resume). On-prem entitlement: `deployment_mode`, `license_valid_until`, `restaurant_installations`, lease fields; ops UI `/ops/licenses`; claim/check-in under `/api/platform/license/*` (install credential). Runtime gate remains `suspended_at` only (ADR-004).
