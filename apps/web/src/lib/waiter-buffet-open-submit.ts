...
export type BuffetOpenSubmitBlockReason = 'empty_snapshot' | 'unchanged' | 'editor_not_ready';
...
export function buffetOpenSubmitBlockReason(
  ...
  if (!hasPositiveBuffetSnapshot(guestSnapshot)) return 'empty_snapshot';
  if (isBuffetSnapshotUnchanged(orders, guestSnapshot)) return 'unchanged';
