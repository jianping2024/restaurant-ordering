...
| 空会话结账 | `empty_session` |
| 自助餐人数未确认（成人+儿童合计为 0） | 400 `guest_count_required`；账单页分单区上方常驻提醒，点击「呼叫结账」toast 同文案且不发请求 |
...
- 人数未确认时：分单区上方提醒一直显示；呼叫结账可点但提示联系服务员改人数，且不进入 `billing`
- 服务员改人数后（成人+儿童 > 0）：提醒消失，可正常呼叫结账
- 呼叫结账后菜单不可加菜
- 结账台「待收」= 折后应收 − 已收合计
- 部分收款后恢复点餐：已收款项不丢失
- 全员付清后会话关闭、桌位可接新客
