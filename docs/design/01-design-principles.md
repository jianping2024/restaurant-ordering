...
- 看板卡片壳：`mesa-scroll-frame` + 上列 modifier；外框统一 `2px`（idle 虚线；边框 `status-*-border` @ ~50%）；正文一律 `brand-text`；**内底唯一写法**为 `rgb(status-*-bg)` 实心（与改造前 `mesa-board-shell-*` 同色），禁止 `[data-theme]` 硬编码 RGB / 禁止平行第二套楼面色板
