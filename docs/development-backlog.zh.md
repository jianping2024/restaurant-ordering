...
| # | 工作项 | 说明 | 源文档 |
|---|--------|------|--------|
| 47 | **Plan 与平台强制功能（若需要）** | 写 `plan`；店级 `feature_flags` 已定为店主唯一可写。平台强制项若做，须独立强制层，**禁止** Ops 再直写同一 jsonb | §4.3 |
...
| **1** | 单机生产栈（**Ubuntu** + Docker Engine + 自托管 Supabase + Web） | ~2–3 周 | Mode B + `install-ubuntu.sh`；Windows/WSL 全栈已废 |
