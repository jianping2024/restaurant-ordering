...
| 名称 | `name` |
| 备注 | `remarks` 截断预览 |
| 桌位 | 已分配数量 + `display_name` 标签预览（默认前 6 个，超出 `+N`）；组内排序经行内「调整顺序」进入 Modal |
| 操作 | 编辑、删除 |
...
由当前 **单一平铺网格** 改为：**待结账快捷区（§6.0.1）+ 横滑楼面 lane tab（同行组入口紧跟最后一桌组）+ 单选内容区**：

```
[ 分组 A ] [ 分组 B ] [ 未分组 ] … [同行组 ▾]  ← 同一横滑条；同行组为下拉（选中 brand-gold）
                                         ├ [创建]  ← 顶区金底主按钮（不随列表滚动）
                                         ├ 组甲 · n 桌
                                         └ 组乙 · n 桌
  ┌──┐ ┌──┐ ┌──┐
...
在 **每个分区内部**（与顶部待结账快捷区内部同一排序入口 `sortWaiterTableCards`）：

1. 有结账请求的桌置顶；
2. 其余按桌位 `sort_order`（再 `display_name`）。**占用/已下单不改变格位**，仅用卡片状态样式表达。

即：**先按分组分区，区内仅结账优先，大厅布局稳定**。
...
| 项 | 现网 | 分组后 | 建议 |
|----|------|--------|------|
| **待结账桌全局置顶** | 全店平铺网格，`allTableCards` **全局**按结账请求 → 活跃 → `sort_order` 排序，待结账桌总在页面最上方 | 分区内仅结账置顶（占用不改格位）；**另增 §6.0.1 顶部待结账快捷区**，跨组汇总全部待结账桌 | **已拍板**：快捷区 + 分区内结账双展示；排序见 §6.2 |
