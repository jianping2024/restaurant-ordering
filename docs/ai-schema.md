...
| `transfer_table_session(restaurant_id, from_table_id, to_table_id, operator_user_id?)` | authenticated, service_role | Move open session between tables; operator via `assert_restaurant_session_operator` (owner_id or active staff waiter\|frontdesk\|cashier\|owner). App enforces `tables.transfer`. |
...
| `assert_restaurant_session_operator(restaurant_id, operator_user_id)` | authenticated, service_role | Transfer/merge operator gate: restaurant `owner_id` or active staff role in waiter\|frontdesk\|cashier\|owner |
