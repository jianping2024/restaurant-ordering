...
| 2b. 托盘卸载（Win Setup） | 唯一入口 `uninstallAgentWithUserData`：清用户配置/日志 → 找卸载命令（Inno `{GUID}}_is1` / DisplayName 前缀 / exe 旁 `unins000.exe`）→ `parseWindowsCommandLine` + `ShellExecute`；找不到才便携提示。本机卸不掉服务器 `print_agent_devices` 占用 |
