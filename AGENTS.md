...
- **Recommended dishes:** sole list `menu_recommended_items` + catalog `recommendedItemIds`; customer strip sentinel `recommended` only — do not add `is_recommended` on `menu_items` or a real `menu_categories` row. Dashboard add is one `POST` `{ menu_item_ids }` (array even for one dish); do not keep a parallel `menu_item_id` add body.
