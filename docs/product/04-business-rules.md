...
**午/晚拆分（概览卡右侧）**：同一 Lisbon 日用唯一分界 `OVERVIEW_DAYPART_CUTOFF_LISBON = 17:00`（`Europe/Lisbon`）切开——`[日始, 17:00)` = 中午，`[17:00, 日末)` = 晚上。营业额按 `closed_at`、订单按 `created_at` 分桶；`dayparts.noon.* + dayparts.evening.*` **等于**对应全日主数（无第三段）。
